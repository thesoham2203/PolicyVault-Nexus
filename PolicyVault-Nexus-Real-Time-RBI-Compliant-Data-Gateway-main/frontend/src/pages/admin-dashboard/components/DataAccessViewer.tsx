...
const DataAccessViewer: React.FC = () => {
  const [consents, setConsents] = useState<Consentss[]>([]);
  const [selectedConsent, setSelectedConsent] = useState<string>('');
  const [isConsentDropdownOpen, setIsConsentDropdownOpen] = useState(false);
  ...
  useEffect(() => {
    const fetchConsents = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:8000/admin/consents`);
  ...
  //       setError(null);
  //       console.log("user")
  //       console.log(currentConsent.user_identifier)
  //       console.log(currentConsent.datafields)
        
  //       currentConsent.datafields.forEach(field => {
  //         formData.append('data_fields', field.trim().toLowerCase());
  //       });
  //       const response = await fetch(
  //         `http://localhost:8000/account-data/${currentConsent.user_identifier}`,
  //         {
  //           method: 'POST',
  //           headers: {
  ...
  //           // body: 
  //           // JSON.stringify({
  //           //   data_fields: currentConsent.datafields
  //           // })
  //           body: JSON.stringify(currentConsent.datafields)
  //         }
  //       );
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:8000/account-data/${currentConsent.user_identifier}`,
          {
            ...
            body: JSON.stringify({
              data_fields: currentConsent.datafields.map(field => field.trim())
            })
          }
        );
  ...
  const isDataVisible = (dataType: string) => {
    if (!currentConsent) return false;
    return currentConsent.datafields.some(field => 
      field.toLowerCase() === dataType.toLowerCase()
    );
  };

  const getConsentStatus = (consent: Consentss) => {
    if (consent.status === 'APPROVED' && consent.status_admin === 'APPROVED') {
      return 'active';
    } else if (consent.status === 'APPROVED' && consent.status_admin === 'REVOKED') {
      return 'revoked';
    } else if (consent.status === 'APPROVED' && consent.status_admin === 'EXPIRED') {
      return 'expired';
    }
    return 'inactive';
  };

  const isActive = currentConsent && 
                  currentConsent.status === 'APPROVED' && 
                  currentConsent.status_admin === 'APPROVED';
  ...
  if (loading.consents && consents.length === 0) {
    return (
      <div className="flex justify-center items-center min-h-screen">
  ...
  if (!currentConsent || !financialData) {
    return <div className="text-center mt-10">Loading consent data...</div>;
  }
            ...
            <div className="">
              ...
              >
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1 text-left">
                    <div className="font-semibold text-gray-900 dark:text-white truncate">
                      {currentConsent?.fiu_id || 'Select Consent'}
                    </div>
                    {currentConsent && (
                      <div className="flex items-center gap-2 mt-1">
                        <StatusChip status={getConsentStatus(currentConsent)} size="sm" />
                ...
                <div className="top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg z-10 max-h-60 overflow-y-auto">
                  {consents.map((consent) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        <StatusChip status={getConsentStatus(consent)} size="sm" />
                        <span className="text-xs text-gray-500 dark:text-gray-400 font-mono bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded truncate max-w-20">
                          {consent.c_id}
                        </span>
                      </div>
              ...
              {consents.map((consent) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="font-semibold text-gray-900 dark:text-white">{consent.fiu_id}</span>
                  </div>
                  <div className="flex items-center justify-between mb-2">
                    <StatusChip status={getConsentStatus(consent)} size="sm" />
                    <span className="text-xs text-gray-500 dark:text-gray-400 font-mono bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">
          ...
          <div className="bg-white/30 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl sm:rounded-2xl shadow-xl border border-gray-200/50 dark:border-gray-700/50">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                ...
                {currentConsent && (
                  <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600 dark:text-gray-400">
                    <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-700 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg">
                      <Shield size={14} />
                      <span className="font-mono truncate">{currentConsent.c_id}</span>
                    </div>
                    <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-700 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg">
                      <Calendar size={14} />
                      <span className="whitespace-nowrap">Created: {new Date(currentConsent.actual_expiry).toLocaleDateString()}</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="p-4 sm:p-6">
              {!currentConsent ? (
                <div className="text-center py-12 sm:py-20">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 rounded-2xl sm:rounded-3xl flex items-center justify-center mx-auto mb-4 sm:mb-6 shadow-lg">
                    <Database size={32} className="text-gray-400 dark:text-gray-500" />
                  </div>
                  <h3 className="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white mb-2 sm:mb-3">No consent selected</h3>
                  <p className="text-gray-600 dark:text-gray-400 text-base sm:text-lg">Select a consent to view the accessible data</p>
                </div>
                ...
                <div className="text-center py-12 sm:py-20">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 rounded-2xl sm:rounded-3xl flex items-center justify-center mx-auto mb-4 sm:mb-6 shadow-lg">
                    <Eye size={32} className="text-gray-400 dark:text-gray-500" />
                  </div>
                  <h3 className="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white mb-2 sm:mb-3">Consent not active</h3>
                  <p className="text-gray-600 dark:text-gray-400 text-base sm:text-lg">
                    This consent is {getConsentStatus(currentConsent)} and cannot access data
                  </p>
                </div>
              ) : (
                <div className="space-y-6 sm:space-y-8">
                    ...
                    <div className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700/50 dark:to-gray-600/50 rounded-lg sm:rounded-xl p-4 sm:p-6 border border-gray-200/50 dark:border-gray-600/50">
                      <div className="grid grid-cols-1 gap-4 sm:gap-6">
                        <div>
                          <label className="text-sm font-semibold text-gray-700 dark:text-gray-300">Account Number</label>
                          <p className="text-sm text-gray-900 dark:text-gray-100 mt-2 font-mono bg-white dark:bg-gray-800 px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 break-all">
                            {financialData ? maskAccountNumber(financialData.account_number) : '••••••••••••'}
                          </p>
                        </div>
                        {isDataVisible('account details') && financialData?.salary_inflow !== undefined && (
                          <div>
                            <label className="text-sm font-semibold text-gray-700 dark:text-gray-300">Account Details</label>
                            <p className="text-sm text-gray-900 dark:text-gray-100 mt-2 bg-white dark:bg-gray-800 px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600">
                              {financialData.account_details}
                            </p>
                          </div>
                        )}
                      </div>
                  ...
                  {/* Balance */}
                  {isDataVisible('account balance') && financialData?.balance !== undefined && (
                    <div>
                      <div className="flex items-center gap-3 mb-3 sm:mb-4">
                  ...
                  {/* Credit Score */}
                  {isDataVisible('credit score') && (financialData?.credit_score ?? null) !== null && (
                    <div>
                      <div className="flex items-center gap-3 mb-3 sm:mb-4">
                        <Activity size={18} className="text-blue-600 dark:text-blue-400" />
                        <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">Credit Score</h3>
                      </div>
                      <div className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700/50 dark:to-gray-600/50 rounded-lg sm:rounded-xl p-6 sm:p-8 border border-gray-200/50 dark:border-gray-600/50">
                        <div className="text-center">
                          <p className="text-2xl sm:text-4xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">
                            {financialData.credit_score}
                          </p>
                          <p className="text-sm text-gray-600 dark:text-gray-400 mt-2 sm:mt-3 font-medium">Credit Score</p>
                        </div>
                      </div>
                    </div>
                  )}

                  {/* Other data fields can be added similarly */}
                  {isDataVisible('salary inflow') && financialData?.salary_inflow !== undefined && (
                    <div>
                      <div className="flex items-center gap-3 mb-3 sm:mb-4">
                        <DollarSign size={18} className="text-purple-600 dark:text-purple-400" />
                        <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">Salary Inflow</h3>
                      </div>
                      <div className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700/50 dark:to-gray-600/50 rounded-lg sm:rounded-xl p-6 sm:p-8 border border-gray-200/50 dark:border-gray-600/50">
                        <div className="text-center">
                          <p className="text-2xl sm:text-4xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
                            {formatCurrency(financialData.salary_inflow)}
                          </p>
                          <p className="text-sm text-gray-600 dark:text-gray-400 mt-2 sm:mt-3 font-medium">Monthly Salary</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {currentConsent.datafields.map(field => {
                    const mappedField = field_mapping[field.toLowerCase()];
                    if (!mappedField || !financialData || !financialData[mappedField as keyof FinancialData]) return null;
                    ...
                    return (
                      <div key={field}>
                        ...
                        <div className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700/50 dark:to-gray-600/50 rounded-lg sm:rounded-xl p-4 sm:p-6 border border-gray-200/50 dark:border-gray-600/50">
                          <p className="text-sm text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800 px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600">
                            {String(financialData[mappedField as keyof FinancialData])}
                          </p>
                        </div>
                      </div>
                    );
