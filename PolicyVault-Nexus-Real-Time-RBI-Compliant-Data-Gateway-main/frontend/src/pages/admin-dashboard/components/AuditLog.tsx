...
interface AuditLogEntry {
  id: string;
  timestamp: string;
  adminEmail: string;  // Changed from fiuName to adminEmail
  consentId: string;
  action: string;
  details: string;
}
...
const AuditLog: React.FC<AuditLogProps> = ({ auditLog = [] }) => {
  ...
  useEffect(() => {
    const fetchAuditLogs = async () => {
      try {
        setLoading(true);
        const response = await fetch('http://localhost:8000/admin/audit-logs');
        if (!response.ok) {
        ...
        const transformedData = result.map((log: any) => ({
          id: log.id,
          timestamp: log.created_at,
          adminEmail: log.email.split('@')[0], // Extract the part before @
          consentId: log.consent_id || 'N/A',
          action: log.action_type.toLowerCase(),
          details: log.action_details?.message || 'No details available'
        }));
  ...
  const getActionIcon = (action: string) => {
    switch (action) {
      ...
      case 'expired':
        return <Clock size={16} className="text-gray-600 dark:text-gray-400" />;
      case 'error':  // Added case for ERROR
  ...
  const filteredLog = data.filter(entry => {
    const matchesSearch = entry.adminEmail.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         entry.consentId.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         entry.action.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg text-gray-700 dark:text-gray-300">Loading audit logs...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center bg-white/60 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200/50 dark:border-gray-700/50 p-8 max-w-md mx-4">
  ...
  return (
    <div className="min-h-screen ">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8">
                ...
                {filteredLog.map((entry) => {
                  ...
                  return (
                    <tr key={entry.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-semibold text-gray-900 dark:text-gray-100">{entry.adminEmail}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900 dark:text-gray-100 font-mono bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-lg">
                          {entry.consentId}
          ...
          {filteredLog.map((entry) => {
            ...
            return (
              <div key={entry.id} className="bg-white/60 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-200/50 dark:border-gray-700/50 p-4 hover:shadow-md transition-all duration-200">
                ...
                <div className="space-y-2">
                  <div>
                    <span className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Admin</span>
                    <div className="text-sm font-semibold text-gray-900 dark:text-gray-100">{entry.adminEmail}</div>
                  </div>
        ...
        {filteredLog.length > 0 && (
          <div className="mt-6 sm:mt-8 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            {['approved', 'revoked', 'rejected', 'error'].map((action) => {
              const count = filteredLog.filter(entry => entry.action === action).length;
              return (
                <div key={action} className="group bg-white/60 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 shadow-xl border border-gray-200/50 dark:border-gray-700/50 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                  <div className="flex items-center gap-2 sm:gap-3">
